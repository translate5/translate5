...
trait editor_Models_Export_FileParser_MQMTrait
{
     ...
     * @var array
     */
    protected $_stackId2ArrayIndex = [];

    /**
     * used by _handleOverlap and subroutines
     * @var array
     */
    protected $_closingButNotOpenedTags = [];

    /**
     * used by _handleOverlap and subroutines
     * @var array
     */
    protected $_openTagsInLoop = [];

    /**
     * used by _handleOverlap and subroutines
     * @var array
     */
    protected $_newTagCounter = [];

    /**
     * used by _handleOverlap and subroutines
     * @var array
     */
    protected $_newOpenTagData = [];

    /**
     * used by _handleOverlap and subroutines
     * @var array
     */
    protected $_newCloseTagData = [];
    ...
    protected function _isImageTag($tag, $type = '')
    {
        ...
        if (in_array($type, ['open', 'close'])) {
            ...
            return (bool) preg_match('"^(' . $type . ' .*)|(.* ' . $type . ')|(.* ' . $type . ' .*)$"', $class);
        }
    ...
    protected function _handleOverlapResolveInsert($correspondingTagId)
    {
        ...
        //insert opening tag
        $insertOpening = ['', $this->_getXliffTagFromData($this->_newOpenTagData, $correspondingTagId)]; //we must also insert an empty text node to keep the rythm of alternating text nodes and tags
        ...
        //insert closing tag
        $insertClosing = [$this->_getXliffTagFromData($this->_newCloseTagData, $correspondingTagId), '']; //we must also insert an empty text node to keep the rythm of alternating text nodes and tags
    ...
    {
        $this->_openTagsInLoop = [];
        $this->_closingButNotOpenedTags = [];
        $this->_stackId2ArrayIndex = [];
        $this->_newOpenTagData = [];
        $this->_newCloseTagData = [];
        reset($this->_stack);
    }
}
