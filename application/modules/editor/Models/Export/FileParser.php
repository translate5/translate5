...
abstract class editor_Models_Export_FileParser {
     ...
     * @return string[]
     */
    private function _generateQmTagData($tag) {
    	$data = [];
    ...
    /**
     * Generates XlifQmTag either by extracting needed data from $tag or by using input $data
     * 
     * @param string $tag
     * @param number $idref
     * @param array $data
     * @return array
     */
    private function _generateXlifQmTag($tag, $idref = 0, $data = []) {
    	if (!is_array($data) || (count($data) < 1)) {
    		$data = $this->_generateQmTagData($tag);
    	}
    	$out = [
    		'tag' => '</mqm:issue>',
    		'data' => $data
    ...
    /**
     * Pushes $item into $this->_stack and if it is an open image tag, also pushes it's id into $this->_idStack
     * 
     * @param string|array $item
     */
    private function _pushStack($item) {
    	$pos = count($this->_stack);
    	array_push($this->_stack, $item);
    	if (is_array($item) && ($item['data']['type'] == 'open')) {
    		array_push($this->_idStack, ['id' => $item['data']['id'], 'pos' => $pos]);
    	}
    }
    
    /**
     * Dumps $this->_stack and appends it to $this->_store; 
     * 
     * On exit both $this->_stack and $this->_idStack are empty
     * 
     */
    private function _addStackToStore() {
    	while (count($this->_stack) > 0) {
    		$item = array_shift($this->_stack);
    		if (is_array($item)) {
    			if (isset($item['prev'])) {
    				$this->_store[] = $item['prev']['tag'];
    			}
    			$this->_store[] = $item['tag'];
    			if (isset($item['next'])) {
    				$this->_store[] = $item['next']['tag'];
    			}
    		} else {
    			$this->_store[] = $item;
    		}
     ...
     * 
     * @param array $gen_close
     * @return boolean
     */
    private function _resolveOverlappedQmTags($gen_close) {
    	$cnt = count($this->_idStack);
    	if ($cnt < 2) return false; // this should never happen normally
    	$last = $this->_idStack[$cnt-1];
    	$prev = $this->_idStack[$cnt-2];
    	$prev_open = $this->_stack[$prev['pos']];
    	if ($prev_open['data']['id'] != $gen_close['data']['id']) return false; // this should never happen normally
    	$idref = $prev_open['data']['id'];
    	$prev_open['data']['id'] .= '-ref';
    	$this->_stack[$last['pos']]['prev'] = $this->_generateXlifQmTag('', 0, ['type' => 'close']);
    	$this->_stack[$last['pos']]['next'] = $this->_generateXlifQmTag('', $idref, $prev_open['data']);
    	array_push($this->_stack, $gen_close);
    	return true;
    }
    
    /**
     * compares $id with the top of $this->_idStack to check while the closed tag matches the open one
     * 
     * @param integer $id
     * @return boolean
     */
    private function _matchTagIds($id) {
    	$cnt = count($this->_idStack);
    	if ($cnt < 1) return false;
    	return ($id == $this->_idStack[$cnt-1]['id']);
    }
    ...
    protected function convertQmTags2XliffFormat($segment){
        ...
        while (count($split) > 0) {
        	...
        	if ($this->_isImageTag($item)) {
        		$gen = $this->_generateXlifQmTag($item);
        		if ($gen['data']['type'] == 'open') {
        			$this->_pushStack($gen);
        		} else {
        			// the normal case
        			if ($this->_matchTagIds($gen['data']['id'])) {
        				$this->_pushStack($gen);
        				$this->_addStackToStore();
        			} else {
        				$this->_resolveOverlappedQmTags($gen);
        			}
        		}
        	} else {
        ...
        // non empty stack means a unclosed image tag
