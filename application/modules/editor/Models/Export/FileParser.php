...
abstract class editor_Models_Export_FileParser {
    ...
    /**
     * Adds to $this->_store a tag generated by $this->_generateXlifQmTag()
     * and returns generated data
     * 
     * @param string $tag
     * @param number $idref
     * @param array $data
    ...
    private function _addXlifQmTag($tag, $idref = 0, $data = []) {
    	$generated = $this->_generateQmTagData($tag, $idref, $data);
    	$this->_store[] = $generated['tag'];
    ...
    private function _addStackToStore() {
    	while (count($this->_stack) > 0) {
    		...
    		if (is_array($item)) {
    			if (isset($item['prev'])) {
    				$this->_store[] = $item['prev'];
    			}
    			$this->_store[] = $item['tag'];
    			if (isset($item['next'])) {
    				$this->_store[] = $item['next'];
    			}
    		} else {
    			$this->_store[] = $item;
    		}
