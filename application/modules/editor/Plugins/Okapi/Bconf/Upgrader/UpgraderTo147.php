    ...
    public static function upgradeFprms(string $bconfDir, string $bconfId, string $bconfName): void
    {
        ...
        foreach ($json['fprm'] as $fprmEntry) {
            ...
            if ($fprm->getType() === Fprm::TYPE_XPROPERTIES) {
                $fprmContents = null;
                if (in_array($okfType, ['okf_openxml', 'okf_idml'])) {
                    $fprmContents = file_get_contents($fprmFile);
                    if (str_contains($fprmContents, 'maxAttributeSize.i=')) {
                        $fprmContents = preg_replace('/maxAttributeSize\.i=\d+/', 'maxAttributeSize.i=41943040', $fprmContents);
                    } else {
                        $fprmContents = rtrim($fprmContents) . "\nmaxAttributeSize.i=41943040";
                    }
                }
                $validation = new PropertiesValidation($fprmFile, $fprmContents);
                $validation->upgrade();
