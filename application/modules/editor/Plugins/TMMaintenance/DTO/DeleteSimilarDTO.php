...
final readonly class DeleteSimilarDTO
{
    public function __construct(
        public SimilarDeleteType $type,
    ...
    public static function fromRequest(Request $request): self
    {
        ...
        $type = SimilarDeleteType::tryFrom($data['type'] ?? '');
        if ($type === null) {
            throw new InexistentSimilarDeleteTypeException();
        }

        return new self(
            SimilarDeleteType::from($data['type']),
            $data['source'],
            $data['target'],
        );
    }
