...
class editor_Plugins_DummyPretranslator_Init extends ZfExtended_Plugin_Abstract
{
    ...
    public function handleAfterImport(Zend_EventManager_Event $event): void
    {
        ...
        if (! $task->isTranslation()) {
            error_log(
                "DummyPretranslator disabled due existing targets for Task "
                . $task->getTaskGuid() . ' - ' . $task->getTaskName()
            );
        ...
        $segments = ZfExtended_Factory::get(editor_Models_Segment_Iterator::class, [$task->getTaskGuid()]);
        foreach ($segments as $segment) {
            /* @var $segment editor_Models_Segment */
            // deu of other pretranslations we have to check targetEdit content here too:
