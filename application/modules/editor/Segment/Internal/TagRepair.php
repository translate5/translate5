    ...
    private function fixStructure(): void
    {
        ...
        for ($i = 0; $i < $this->numCheckTags; $i++) {
            ...
            if ($tag->isSingle()) {
                ...
                $this->resultTags[] = $tag;
            } elseif ($tag->counterpart === null) {
                // remove incomplete tags
                $this->numRemoved++;
            } elseif ($tag->isOpening() && ! $this->isStructurallyValid($this->checkTags, $i)) {
        ...
        if ($numBroken > 0) {
            $this->findBoundries();
            usort($this->resultTags, [$this->fieldTags, 'compare']);
        ...
        if ($this->hadErrors()) {
            ...
            usort($all, [$this->fieldTags, 'compare']);
