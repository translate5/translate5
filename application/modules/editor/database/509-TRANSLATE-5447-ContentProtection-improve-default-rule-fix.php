...
$SCRIPT_IDENTIFIER = '508-TRANSLATE-5447-ContentProtection-improve-default-rule-fix.php';

$db = Zend_Db_Table::getDefaultAdapter();

$names = [
    'default with "\'" separator' => <<<REGEX
/(\s|^|\()([±\-+]?((([1-9]\d{0,2}\.){1}(\d{3}\.)*\d{3})|(([1-9]\d{0,2})|0))'\d+)(([\.,;:?!](\s|$))|\s|$|\))/u
REGEX,
...
/(\s|^|\()([±\-+]?(((\d{1,4},){1}(\d{4},)*\d{4})|(([1-9]\d{0,3})|0))\.\d+)(([\.,;:?!](\s|$))|\s|$|\))/u
REGEX,
...
/(\s|^|\()([±\-+]?((([1-9]\d{0,2}\x{2009}){1}(\d{3}\x{2009})*\d{3})|(([1-9]\d{0,2})|0))\.\d+)(([\.,;:?!](\s|$))|\s|$|\))/u
REGEX,
    'default with [NNBSP] thousand decimal dot' => <<<REGEX
/(\s|^|\()([±\-+]?((([1-9]\d{0,2}\x{202F}){1}(\d{3}\x{202F})*\d{3})|(([1-9]\d{0,2})|0))\.\d+)(([\.,;:?!](\s|$))|\s|$|\))/u
REGEX,
