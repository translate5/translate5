...
UPDATE LEK_content_protection_content_recognition SET regex = '/(\\s|^|\\()([-+]?[1-9]\\d{0,2}(\\s)(\\d{3}\\3)*\\d{3})((\\.(\\s|$))|(,(\\s|$))|\\s|$|\\))/u' WHERE type = 'integer' AND name = 'default generic with whitespace';
