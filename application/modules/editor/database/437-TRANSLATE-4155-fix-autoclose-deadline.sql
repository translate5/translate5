...
VALUES ('runtimeOptions.import.projectDeadline.jobDeadlineFraction', 1, 'editor', 'import', 100, 100, null, 'integer', null, 'This setting is only for API-created Jobs (special flag "calculateDeadlineDate" sent) and overwrites any user assignment defaults. It determines the percentage by which the users jobs will be closed before the project-deadline (if one is set). 100% means, the job deadline equals the project-deadline. Do not set to 0, which would immediately finish the job after creation.', 4, 'none', 'Project Deadline: Auto-Close Deadline Configuration', 'Workflow', null);

INSERT INTO Zf_configuration (`name`,`confirmed`,`module`,`category`,`value`,`default`,`defaults`,`type`,`typeClass`,`description`,`level`,`accessRestriction`,`guiName`,`guiGroup`,`comment`)
VALUES ('runtimeOptions.workflow.autoCloseJobs', 1, 'editor', 'import', 0, 0, null, 'boolean', null, 'When set, all jobs for a project with a job-deadline set will be auto-closed when overdue.', 16, 'none', 'Auto-Close Jobs', 'Workflow', null);
