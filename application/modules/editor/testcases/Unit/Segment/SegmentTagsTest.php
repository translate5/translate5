...
class SegmentTagsTest extends SegmentTagsTestAbstract
{
    ...
    public function testTagStripping(): void
    {
        ...
        $expectedReplaceAll = '~ITAG~HOSTED~ITAG~~ITAG~~ITAG~Team Basic~ITAG~~ITAG~~ITAG~Team Visual~ITAG~~ITAG~~ITAG~Community Member~ITAG~ ~ITAG~~ITAG~ON PREMISE~ITAG~~ITAG~~ITAG~Free~ITAG~~ITAG~~ITAG~Community Member Basic~ITAG~~ITAG~~ITAG~Community Member Visual~ITAG~';
        $replaced = editor_Segment_Internal_Tag::replaceInternalTags($markup, '~ITAG~');
        $this->assertEquals($expectedReplaceAll, $replaced);
        // test stripping/replacing single tags
        $expectedReplaceSingle = '~ITAG~HOSTED~ITAG~~STAG~~ITAG~Team Basic~ITAG~~STAG~~ITAG~Team Visual~ITAG~~STAG~~ITAG~Community Member~ITAG~ ~STAG~~ITAG~ON PREMISE~ITAG~~STAG~~ITAG~Free~ITAG~~STAG~~ITAG~Community Member Basic~ITAG~~STAG~~ITAG~Community Member Visual~ITAG~';
        $replaced = editor_Segment_Internal_Tag::replaceSingleInternalTags($markup, '~STAG~');
        $replaced = editor_Segment_Internal_Tag::replaceInternalTags($replaced, '~ITAG~');
        $this->assertEquals($expectedReplaceSingle, $replaced);
        // test stripping/replacing single tags with wrong class-order
        $markup = str_replace('<div class="single 70682069643d2232223e266c743b62723e3c2f7068 internal-tag ownttip">', '<div class="70682069643d2232223e266c743b62723e3c2f7068 internal-tag single ownttip">', $markup);
        $replaced = editor_Segment_Internal_Tag::replaceSingleInternalTags($markup, '~STAG~');
        $replaced = editor_Segment_Internal_Tag::replaceInternalTags($replaced, '~ITAG~');
        $this->assertEquals($expectedReplaceSingle, $replaced);
    }
    ...
    public function testRealDataTags1(): void
    {
        ...
        $replacedLabeled = '[edit] Last updated:';
    ...
    public function testRealDataTags2(): void
    {
        ...
        $replacedLabeled = 'cd httpd-2_x_NN↵./configure --enable-so↵make↵make install';
        $this->createOriginalDataTest($segmentId, $original, $markup, $replacedLabeled);
    }
    ...
    public function testRealDataTags3(): void
    {
        ...
        $replacedLabeled = '&lt;FilesMatch \.php$&gt;↵ ·SetHandler application/x-httpd-php↵&lt;/FilesMatch&gt;';
        $this->createOriginalDataTest($segmentId, $original, $markup, $replacedLabeled);
    }
    ...
    public function testRealDataTags4(): void
    {
        ...
        $replacedLabeled = 'Lighttpd 1.4 on Unix systems Apache 1.3.x on Unix systems';
        $this->createOriginalDataTest($segmentId, $original, $markup, $replacedLabeled);
    }
    ...
    public function testRealDataTags5(): void
    {
        ...
        $replacedLabeled = '&lt;FilesMatch "\.phps$"&gt;↵ ·SetHandler application/x-httpd-php-source↵&lt;/FilesMatch&gt;';
        $this->createOriginalDataTest($segmentId, $original, $markup, $replacedLabeled);
    }
    ...
    public function testRealDataTags8(): void
    {
        ...
        $replacedLabeled = 'Aktualizacja 07-2016 (akt. 01)';
        $this->createOriginalDataTest($segmentId, $original, $markup, $replacedLabeled);
    }
    ...
    {
        // testing "real" segment content
        // NOTE: when TagSequence is in DEBUG-mode the duplicatesavecheck tag will throw a warning that the UNPARSED HTML DOM DOES NOT MATCH. This is due to the missing "/" in the self-closing duplicatesavecheck-img"
        $segmentId = 688445;
        ...
        $replacedLabeled = 'für Industriekunden';
        $this->createOriginalDataTest($segmentId, $original, $markup, $replacedLabeled);
    }
    ...
    public function testRealDataTags11(): void
    {
        // testing "real" segment content that consist of a sequence of <ins> from different users most of zero text-length
        $segmentId = 688449;
        $markup = '<ins class="trackchanges ownttip" data-usertrackingid="70686" data-usercssnr="usernr2" data-workflowstep="workflowEnded2" data-timestamp="2025-09-09T16:59:26+02:00">Range:<div class="single  number internal-tag ownttip"><span class="short" title="&lt;5/&gt; CP: default simple">&lt;5/&gt;</span><span class="full" data-originalid="number" data-length="3" data-source="721" data-target="721"></span></div><div class="single 70682069643d2234222063747970653d22782d726566223e26616d703b6e6273703b3c2f7068 internal-tag ownttip"><span class="short" title="&lt;ph id=&quot;4&quot; ctype=&quot;x-ref&quot;&gt;&amp;amp;nbsp;&lt;/ph&gt;">&lt;4/&gt;</span><span class="full" data-originalid="4" data-length="-1">&lt;ph id="4" ctype="x-ref"&gt;&amp;amp;nbsp;&lt;/ph&gt;</span></div><div class="open 6270742069643d2232222063747970653d22782d7363686172223e266c743b736368617220666f6e743d2671756f743b4d657461506c75734c462671756f743b206e723d2671756f743b323032362671756f743b207472616e736c6174653d2671756f743b6e6f2671756f743b2667743b3c2f627074 internal-tag ownttip"><span class="short" title="&lt;bpt id=&quot;2&quot; ctype=&quot;x-schar&quot;&gt;&amp;lt;schar font=&amp;quot;MetaPlusLF&amp;quot; nr=&amp;quot;2026&amp;quot; translate=&amp;quot;no&amp;quot;&amp;gt;&lt;/bpt&gt;">&lt;2&gt;</span><span class="full" data-originalid="2" data-length="-1">&lt;bpt id="2" ctype="x-schar"&gt;&amp;lt;schar font=&amp;quot;MetaPlusLF&amp;quot; nr=&amp;quot;2026&amp;quot; translate=&amp;quot;no&amp;quot;&amp;gt;&lt;/bpt&gt;</span></div></ins><ins class="trackchanges ownttip" data-usertrackingid="1153" data-usercssnr="usernr1" data-workflowstep="no workflow1" data-timestamp="2026-01-26T13:09:16+02:00"><div class="close 6570742069643d2232223e266c743b2f73636861722667743b3c2f657074 internal-tag ownttip"><span class="short" title="&lt;ept id=&quot;2&quot;&gt;&amp;lt;/schar&amp;gt;&lt;/ept&gt;">&lt;/2&gt;</span><span class="full" data-originalid="2" data-length="-1">&lt;ept id="2"&gt;&amp;lt;/schar&amp;gt;&lt;/ept&gt;</span></div></ins><ins class="trackchanges ownttip" data-usertrackingid="70686" data-usercssnr="usernr2" data-workflowstep="workflowEnded2" data-timestamp="2025-09-09T16:59:26+02:00"><div class="single 70682069643d2233223ee280a63c2f7068 internal-tag ownttip"><span class="short" title="…">&lt;3/&gt;</span><span class="full" data-originalid="3" data-length="-1">…</span></div><div class="single  number internal-tag ownttip"><span class="short" title="&lt;6/&gt; CP: default simple">&lt;6/&gt;</span><span class="full" data-originalid="number" data-length="4" data-source="1805" data-target="1805"></span></div></ins><ins class="trackchanges ownttip" data-usertrackingid="70926" data-usercssnr="usernr3" data-workflowstep="workflowEnded2" data-timestamp="2025-09-16T08:52:29+02:00"><div class="single 70682069643d2231222063747970653d22782d726566223e26616d703b6e6273703b3c2f7068 internal-tag ownttip"><span class="short" title="&lt;ph id=&quot;1&quot; ctype=&quot;x-ref&quot;&gt;&amp;amp;nbsp;&lt;/ph&gt;">&lt;1/&gt;</span><span class="full" data-originalid="1" data-length="-1">&lt;ph id="1" ctype="x-ref"&gt;&amp;amp;nbsp;&lt;/ph&gt;</span></div></ins>';
        $this->createDataTest($segmentId, $markup);
    }
