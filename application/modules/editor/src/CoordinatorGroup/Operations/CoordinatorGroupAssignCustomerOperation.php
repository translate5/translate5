...
final class CoordinatorGroupAssignCustomerOperation implements CoordinatorGroupAssignCustomerOperationInterface
{
    ...
    public function assignCustomer(CoordinatorGroup $group, Customer $customer): void
    {
        if (! $group->isTopRankGroup()) {
            $parentGroup = $this->coordinatorGroupRepository->get((int) $group->getParentId());
            $this->coordinatorGroupCustomerAssociationValidator->assertCustomersAreSubsetForCoordinatorGroup(
                (int) $parentGroup->getId(),
                (int) $customer->getId()
            );
        }

        $groupCustomer = $this->coordinatorGroupRepository->getEmptyCoordinatorGroupCustomerModel();
        $groupCustomer->setGroupId((int) $group->getId());
        $groupCustomer->setCustomerId((int) $customer->getId());

        $this->coordinatorGroupRepository->saveCustomerAssignment($groupCustomer);
    }
}
