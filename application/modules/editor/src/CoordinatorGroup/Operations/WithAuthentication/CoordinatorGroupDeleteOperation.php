...
final class CoordinatorGroupDeleteOperation implements CoordinatorGroupDeleteOperationInterface
{
    public function __construct(
        private readonly CoordinatorGroupDeleteOperationInterface $operation,
    ...
    {
        return new self(
            \MittagQI\Translate5\CoordinatorGroup\Operations\CoordinatorGroupDeleteOperation::create(),
    ...
    public function deleteCoordinatorGroup(CoordinatorGroup $group): void
    {
        ...
        try {
            $this->permissionAssert->assertGranted(CoordinatorGroupAction::Delete, $group, new PermissionAssertContext($authUser));
            ...
            $this->logger->info(
                ...
                [
                    'message' => sprintf(
                        'Attempt to delete Coordinator Group (name: "%s") by AuthUser (guid: %s) was granted',
                        $group->getName(),
            ...
            $this->logger->warn(
                ...
                [
                    'message' => sprintf(
                        'Attempt to delete Coordinator Group (name: "%s") by AuthUser (guid: %s) was not granted',
                        $group->getName(),
                        $authUser->getUserGuid()
                    ),
                    'coordinatorGroup' => $group->getName(),
