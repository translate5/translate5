...
class WorkflowStepConfigType extends ZfExtended_DbConfig_Type_CoreTypes
{
    /**
    ...
    public function isValidInDefaults(editor_Models_Config $config, string $value): bool
    {
        ...
        $diffFound = array_diff($values, self::getAllWorkflowSteps());
    ...
    {
        return join(',', self::getAllWorkflowSteps());
    }

    private static function getAllWorkflowSteps(): array
    {
        $wfStep = new editor_Models_Workflow_Step();

        return array_unique(array_column($wfStep->loadAll(), 'name'));
    }
}
