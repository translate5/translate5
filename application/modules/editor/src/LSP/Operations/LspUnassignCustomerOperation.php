...
final class LspUnassignCustomerOperation implements LspUnassignCustomerOperationInterface
{
    ...
    private function deleteAssociationWithDependencies(LanguageServiceProvider $lsp, Customer $customer): void
    {
        ...
        $defaultLspJobs = $this->defaultLspJobRepository->getDefaultLspJobsOfLspForCustomer(
            (int) $lsp->getId(),
            (int) $customer->getId(),
        );
        ...
        $defaultUserJobs = $this->defaultUserJobRepository->getDefaultJobsOfCustomerForUsersOfLsp(
            (int) $customer->getId(),
            (int) $lsp->getId(),
        );
