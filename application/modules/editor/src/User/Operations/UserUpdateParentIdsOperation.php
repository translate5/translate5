...
final class UserUpdateParentIdsOperation
{
     ...
     */
    public function setParentIdsOnUserCreationBy(User $user, ?string $parentId, User $authUser): void
    {
    ...
    {
        $parentUser = $this->resolveParentUser($parentId, $authUser, false);

        $this->updateParentIds($user, $this->getParentIds($parentUser));
    }

    private function resolveParentUser(?string $parentId, User $authUser, bool $creationBranch = true): ?User
    {
        if ($this->canSeeAllUsers($authUser) && ! empty($parentId)) {
            return $this->resolveUser($parentId);
        } elseif ($creationBranch) {
            return $authUser;
        }
    ...
    private function resolveUser(string $identifier): User
    {
        try {
            if (is_numeric($identifier)) {
                return $this->userRepository->get((int) $identifier);
            }

            return $this->userRepository->getByGuid($identifier);
        } catch (ZfExtended_Models_Entity_NotFoundException) {
            throw new ProvidedParentIdCannotBeEvaluatedToUserException($identifier);
        }
    }
