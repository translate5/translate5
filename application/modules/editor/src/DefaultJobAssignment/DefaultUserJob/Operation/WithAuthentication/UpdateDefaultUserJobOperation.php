...
class UpdateDefaultUserJobOperation implements UpdateDefaultUserJobOperationInterface
{
    public function __construct(
        ...
        private readonly UpdateDefaultUserJobOperationInterface $operation,
    ...
    public static function create(): self
    {
        return new self(
            ...
            new UserRepository(),
            \MittagQI\Translate5\DefaultJobAssignment\DefaultUserJob\Operation\UpdateDefaultUserJobOperation::create(),
            DefaultUserJobActionPermissionAssert::create(),
            Zend_Registry::get('logger')->cloneMe('defaultUserJob.update'),
        );
    }
    ...
    public function updateJob(DefaultUserJob $job, UpdateDefaultJobDto $dto): void
    {
        ...
        try {
            ...
            $this->logger->info(
                ...
                [
                    'message' => sprintf(
                        'Attempt to update default user job (id: "%s") by AuthUser (guid: %s) was granted',
                        $job->getId(),
            ...
            $this->logger->info(
                ...
                [
                    'message' => sprintf(
                        'Attempt to update default user job (id: "%s") by AuthUser (guid: %s) was denied',
                        $job->getId(),
                        $authUser->getUserGuid(),
                    ),
                    'lspJobId' => $job->getId(),
