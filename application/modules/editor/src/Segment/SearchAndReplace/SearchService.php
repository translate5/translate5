...
class SearchService
{
     ...
     */
    public function search(SearchQueryDto $query): array
    {
        $sfm = $this->segmentFieldManagerFactory->getManager($query->taskGuid);

        $sortColMap = $sfm->getSortColMap();

        $filter = new editor_Models_Filter_SegmentSpecific();
        ...
        return (array) $segment->search($query->toArray());
    }
}
