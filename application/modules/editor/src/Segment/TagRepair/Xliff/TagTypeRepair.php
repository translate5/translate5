...
class TagTypeRepair implements RepairInterface
{
    /**
     * Apply tag type fixes
     *
    ...
    public function apply(string $text, array $sourceTags, array $translatedTags): string
    {
        ...
        foreach ($translatedTags as $tag) {
            ...
            // if source has this tag id, check if type matches
            if (isset($sourceTagsById[$id])) {
                $sourceType = $sourceTagsById[$id]->getType();
                $translatedType = $tag->getType();

                // if types dont match, fix it
                if ($sourceType !== $translatedType) {
                    // create corrected tag
                    $newTag = $tag->cloneWithChanges([
                        'type' => $sourceType,
                        'rid' => $sourceTagsById[$id]->getRid(),
                    ]);
