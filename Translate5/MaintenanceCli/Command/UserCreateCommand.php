...
class UserCreateCommand extends UserAbstractCommand
{
    ...
    protected function configure()
    {
        ...
        // the "--help" option
        ->setHelp('Creates a translate5 user via CLI - see arugments and options .');
        ...
        $this->addOption(
            ...
            'Give one or multiple roles for the user. Valid values are: '.join(', ', $this->allRoles),
            []);
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $userModel->setRoles(array_merge(array_intersect($this->input->getOption('roles'), $this->allRoles), self::ROLES_FIXED));
        $userModel->validate();
        try {
            $userModel->save();
        }
