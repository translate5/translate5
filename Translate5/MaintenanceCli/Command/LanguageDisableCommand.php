...
class LanguageDisableCommand extends Translate5AbstractCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setDescription('Disables a language.')

            // the full command description shown when running the command with
            // the "--help" option
            ->setHelp('Disables a language - ensure that you know what you are doing when using that command!');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($byRfc5646 && strlen($languageId) === 2) {
            $languageIds = LanguageVisibility::updateGroupVisibility($languageId, true);
        } else {
