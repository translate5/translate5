    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (! is_dir($defaultBconfDir) || ! is_readable($defaultBconfDir)) {
            if (is_dir($defaultBconfDir) && ! is_readable($defaultBconfDir)) {
                ...
                $bconfEntity->db->update([
                    'name' => ZfExtended_Utils::uuid(),
                    'isDefault' => 0,
                ], [
                    'id = ?' => $oldDefaultBconfId,
                ]);
                ...
                $taskMetaEntity->db->update([
                    'bconfId' => $defaultBconfId,
                ], [
                    'bconfId = ?' => $oldDefaultBconfId,
                ]);
                // now delete old invalid bconf
                $bconfFilterEntity->db->delete([
                    'bconfId = ?' => $oldDefaultBconfId,
                ]);
                $bconfEntity->db->delete([
                    'id = ?' => $oldDefaultBconfId,
                ]);
