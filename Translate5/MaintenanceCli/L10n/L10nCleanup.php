...
use MittagQI\ZfExtended\FileDeleteException;
...
class L10nCleanup
{
    ...
    public function process(): int
    {
        ...
        foreach (L10nHelper::getAllLocales() as $locale) {
            $zxliffs = array_merge($zxliffs, $this->collectAllXliffs($locale));
        }
        foreach ($zxliffs as $zxliff) {
            $xliffPath = substr($zxliff, 0, -1 * strlen(Localization::FILE_EXTENSION)) . 'xliff';
            if (file_exists($xliffPath)) {
                if (! unlink($xliffPath)) {
                    throw new FileDeleteException($xliffPath);
                }
                $removed++;
            }
        }
