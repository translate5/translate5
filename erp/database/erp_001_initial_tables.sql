...
# Export von Tabelle ERP_customer
# ------------------------------------------------------------
...
# Export von Tabelle ERP_keyaccount
# ------------------------------------------------------------
...
DELIMITER |
	CREATE  TRIGGER `ERP_order_versioning` BEFORE UPDATE ON `ERP_order`
		FOR EACH ROW
...
# Export von Tabelle ERP_order_comment
# ------------------------------------------------------------
...
# Export von Tabelle ERP_order_history
# ------------------------------------------------------------
...
# Export von Tabelle ERP_purchaseOrder
# ------------------------------------------------------------
...
CREATE TABLE `ERP_purchaseOrder` (
  ...
  `modifiedDate` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `orderId_number` (`orderId`,`number`),
  KEY `orderId` (`orderId`),
  KEY `number` (`number`),
...
DELIMITER |
	CREATE  TRIGGER `ERP_purchadeOrder_versioning` BEFORE UPDATE ON `ERP_purchaseOrder`
		FOR EACH ROW
			IF OLD.entityVersion = NEW.entityVersion THEN 
				SET NEW.entityVersion = OLD.entityVersion + 1;
			ELSE 
...
# Export von Tabelle ERP_purchaseOrder_comment
# ------------------------------------------------------------
...
# Export von Tabelle ERP_purchaseOrder_history
# ------------------------------------------------------------
