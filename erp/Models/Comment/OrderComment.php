...
/**
 * ERP-OrderComment Entity Objekt
 * 
 ...
 * 
*/
class erp_Models_Comment_OrderComment extends erp_Models_Comment_Abstract {
    protected $dbInstanceClass = 'erp_Models_Db_OrderComment';
    ...
    */
    public function updateOrder($orderId) {
        ...
        $comments = $this->loadByForeignId($orderId);
        
        $order->setComments($this->getMarkedUp($comments));
        $order->save();
    }
    
    /**
     * adds the order id filter to the comment is editable check
     * (non-PHPdoc)
     * @see erp_Models_Comment_Abstract::whereForeignId()
     */
    protected function whereForeignId(Zend_Db_Select $select) {
        return $select->where('orderId = ?', $this->getOrderId());
    }
